...
interface ITourLocation extends IGeoLocation {
  day?: number;
}
...
tourSchema.pre(/^find/, function (this: mongoose.Query<unknown, unknown> & { start?: number }, next) {
  this.find({ secretTour: { $ne: true } });
  this.start = Date.now();
  next();
});

tourSchema.pre(/^find/, function (this: mongoose.Query<unknown, unknown>, next) {
  this.populate({
    path: 'guides',
...
tourSchema.post(/^find/, function (this: mongoose.Query<unknown, unknown> & { start?: number }, docs: unknown, next: () => void) {
