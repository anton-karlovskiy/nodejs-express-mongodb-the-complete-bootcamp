...
export const getOne = (Model: Model<any>, popOptions?: PopulateOptions): RequestHandler =>
  catchAsync(async (req, res, next) => {
...
export const getAll = (Model: Model<any>): RequestHandler =>
  catchAsync(async (req, res, _next) => {
    ...
    const features = new APIFeatures(Model.find(filter), req.query as Record<string, string>)
      .filter()
      .sort()
      .limitFields()
      .paginate();
    ...
    res.status(200).json({
      status: 'success',
      results: (docs as unknown[]).length,
      data: { data: docs }
    });
  });
